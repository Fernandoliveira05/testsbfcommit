...
| 07    | **Requisição de Chamado pela Interface Ubidots**                                                      | O Clephost deve estar configurado com Wi-Fi e conectado ao broker MQTT, com token correto do Ubidots no código. Além disso, um computador com acesso à conta do sistema deve estar logado no Ubidots. | O responsável pelo gerenciamento realiza uma solicitação para o Clephost mais próximo através do dashboard Ubidots. | O Clephost recebe a mensagem, processa o chamado e exibe no LCD: **"Chamado Recebido"**. O médico aceita ou recusa pelo botão físico. | O LCD exibiu a mensagem corretamente e o sistema atualizou o status no dashboard após o médico pressionar um dos botões.                                                             |
| 08    | **Exibição de Informações no Visor LCD**                                                              | O ESP32 deve estar configurado com visor LCD conectado via I2C e comunicação MQTT ativa.                         | O usuário do sistema envia dados de localização e categoria da urgência para um ESP32 específico através do dashboard. | O LCD exibe os dados configurados: localização e urgência (leve, grave ou urgente).                                                  | Os dados foram exibidos corretamente no LCD, mostrando a localização e categoria da urgência configurada no sistema.                                                                 |
